...
function ltebandselection() {
    ...
    if (band && (band = band.toLowerCase()), null != band && "" !== band) {
        var bs = band.split("+"),
            ltesum = 0;
        if (all_bands = "0xA3E2AB0908DF", "AUTO" === band.toUpperCase()) ltesum = all_bands;
        else {
...
function i2() {
    if (ca_txt = wan_active_band + " - PCI,EARFCN:" + parseInt(lte_pci, 16) + "," + wan_active_channel, "" != signal.lte_multi_ca_scell_info) {
        ...
        for (var i = 0; i < ca_v.length; i++) d = ca_v[i].split(","), b = d[3], e = d[4], p = d[1], ca_txt += "\nB" + b + " - PCI,EARFCN:" + p + "," + e
    }
    ca_txt += "\n\n" + nr5g_action_band + " - PCI:" + parseInt(nr5g_pci, 16) + " - EARFCN:" + nr5g_action_channel, alert(ca_txt)
}

function setdns() {
    var dns = prompt("Please input 2 dns servers, separated by \",\"  (example 1.1.1.1,1.0.0.1).If you want to use PROVIDER settings, write 'AUTO'.", "AUTO");
    if (dns && (dns = dns.toLowerCase()), null != dns && "" !== dns) {
        var dnss = dns.split(",");
            ...
            success: function(data) {
                ad = cookWithRequest(cookWithRequest(data.wa_inner_version + data.cr_version) + data.RD), $.ajax({
                    ...
                    data: {
                        ...
                        pdp_type: "IP",
                        dns_mode: "manual",
                        prefer_dns_manual: dnss[0],
                        standby_dns_manual: dnss[1],
                        index: 1,
                        AD: ad
                    },
                    success: function(d) {
...
gw = 500, gh = 30, gt = 3, signal = "", version = "888-4.0", [arsrp, arsrq, asinr, anr5rsrp, anr5sinr] = [
    ...
    [],
    []
], $("#txtUserName").attr("maxlength", "100"), console.log("Code by Miononno - v" + version), console.log("type: signal"), window.setInterval(getStatus, 1500), $("#change").prop("disabled", !1);
